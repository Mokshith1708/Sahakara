...
export async function uploadService(file) {
  if (!file) return;
  ...
  try {
    const uploadedFile = await storage.createFile(
      config.serviceStorageId,
      ID.unique(),
      asset
    );

    const fileUrl = await getServicePreview(uploadedFile.$id);
    return fileUrl;
  } catch (error) {
    throw new Error(error);
  }
...
export async function getServicePreview(fileId) {
  try {
    const fileURI = storage.getFileView(config.serviceStorageId, fileId);
...
export const createBlog = async ({ authorId, author, content, photo }) => {
  try {
    const blogURI = await uploadBlog(photo);
...
export const createItem = async ({
  ...
  try {
    const itemURI = await uploadItem(photo);
...
export const createService = async ({ creatorId, creator, content, service }) => {
  try {
    const serviceURI = await uploadService(service);
...
export const changePassword = async ({
        ...
        try {
          console.log(newPassword,oldPassword," pass");
          const result =  await account.updatePassword(password=newPassword,oldPassword=oldPassword);
          console.log(result);
          return result;
        } catch (error) {
          throw new Error(error);
