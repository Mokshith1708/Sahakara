...
const Item = ({ itemId, photoURI, itemName, price, description, userId, location }) => {
  const handlePress = () => {
    router.push({
      pathname: `/queries/${itemId}`,
      params: { itemId, photoURI, itemName, price, description, userId, location },
    });
  };
  return (
    <TouchableOpacity
    ...
    >
      <View className="w-1/2 border-r-2 border-r-white overflow-hidden">
        <Image source={{ uri: photoURI }} resizeMode="contain" className="w-full h-36" />
      </View>
